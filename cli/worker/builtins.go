...
func (w *workerS) builtinHelps(app cli.App, p *cli.Command) { //nolint:revive
	app.NewCmdFrom(p, func(b cli.CommandBuilder) {
		b.Titles("help", "h", "info", "usage", "__completion", "__complete").
			...
			Group(cli.SysMgmtGroup).
			Hidden(true, false).
			OnMatched(func(c *cli.Command, position int, hitState *cli.MatchState) (err error) {
				w.inCompleting = true
...
$ {{.AppName}} gen man
		...
		bb.AddCmd(func(b1 cli.CommandBuilder) {
			b1.Titles("manual", "m", "man").
			...
			b1.AddFlg(func(b cli.FlagBuilder) {
				b.Titles("dir", "d").
					Default("").
		...
		bb.AddCmd(func(b1 cli.CommandBuilder) {
			b1.Titles("doc", "d", "docx", "tex", "pdf", "markdown").
		...
		bb.AddCmd(func(b1 cli.CommandBuilder) {
			b1.Titles("shell", "s", "sh", "bash", "zsh", "fish", "elvish", "fig", "powershell", "ps").
