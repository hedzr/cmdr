...
func TestSingleCommandLine1(t *testing.T) {
	...
	_ = cmdr.Exec(rootCmdForTesting(),
		...
		cmdr.WithAlterLocations(),
		cmdr.WithConfigSubDirAutoName("conf.d"),
		cmdr.WithSearchAlterConfigFiles(true),
		cmdr.WithWarnForUnknownCommand(false),
		cmdr.WithHelpScreenHooks(nil, nil),
		cmdr.WithConfigFileLoadingHooks(nil, nil),
...
func TestBacktraceCmdNames(t *testing.T) {
	...
	// copyRootCmd = rootCmdForTesting
	commands := []struct{ cmdline, result, verboseResult string }{
	...
	for _, cc := range commands {
		...
		if last, err := cmdr.Worker().InternalExecFor(rootCmdForTesting(), os.Args); err != nil {
			...
			r1 := cmdr.BacktraceCmdNamesForTest(last, false)
			vr1 := cmdr.BacktraceCmdNamesForTest(last, true)
			if cc.result != r1 {
...
// testing args
var execTestings = map[string]func(t *testing.T, c *cmdr.Command, e error) error{
